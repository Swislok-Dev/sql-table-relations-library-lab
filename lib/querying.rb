...
def select_value_and_count_of_most_prolific_species
  "SELECT characters.species, COUNT(characters.species) AS counted_species 
  FROM characters 
  GROUP BY characters.species 
  ORDER BY counted_species DESC LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) AS number_of_books
  FROM characters
